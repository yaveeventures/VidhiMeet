...
function bookingView() {
  ...
  if (s === 3) {
    ...
    let originalTotal = fee + p;
    let discountAmount = 0;
    let discountPercent = 0;
    let finalTotal = originalTotal;

    if (booking.voucherDiscountPercent) {
      discountPercent = booking.voucherDiscountPercent;
    ...
    content.innerHTML = `
      ...
      <div class="summary">
        ...
        ${discountPercent > 0 ? `
          <div class="row" style="color: var(--forest); font-weight: 700;">
            <span>Promotional Discount (${discountPercent}% OFF)</span>
            <span>-${money(discountAmount)}</span>
          </div>
        ` : ''}
        <div class="row total"><span>Total</span><span>${money(finalTotal)}</span></div>
      </div>

      <!-- Promo Voucher Input Box -->
      <div class="voucher-box" style="margin: 14px 0 16px; background: #f8fafc; border: 1.5px dashed ${booking.voucherCode ? '#a7f3d0' : '#cbd5e1'}; border-radius: 12px; padding: 12px 16px;">
        <div style="display: flex; gap: 8px; align-items: center;">
          <input type="text" id="booking-voucher-input" placeholder="Promotional code (optional)" value="${booking.voucherCode || ''}" style="flex: 1; text-transform: uppercase; font-family: monospace; font-weight: 700; padding: 9px 12px; border: 1px solid var(--line); border-radius: 8px; font-size: 14px; letter-spacing: 0.5px;">
          ${booking.voucherCode ? `
            <button type="button" id="btn-remove-voucher" class="ghost secondary" style="padding: 9px 14px; font-size: 13px; font-weight: 600; color: #dc2626;">Remove</button>
          ` : `
            <button type="button" id="btn-apply-voucher" class="secondary" style="padding: 9px 16px; font-size: 13px; font-weight: 700; white-space: nowrap; background: #173c30; color: #fff; border: none; border-radius: 8px; cursor: pointer;">Apply</button>
          `}
        </div>
        <div id="voucher-feedback" style="margin-top: 6px; font-size: 12px; font-weight: 600; min-height: 16px; color: ${booking.voucherCode ? 'var(--forest)' : '#64748b'};">
          ${booking.voucherCode ? `✓ Promo code <strong>${booking.voucherCode}</strong> applied (${booking.voucherDiscountPercent}% discount)!` : 'Have a coupon or discount voucher? Enter it above.'}
        </div>
      </div>
      ...
      <div class="actions">
        ...
        <button class="primary" id="pay" ${booking.disclaimerChecked ? '' : 'disabled'}>Confirm & pay ${money(finalTotal)}</button>
      </div>
    `;
    ...
    if (ack) {
      ack.onchange = (e) => {
        booking.disclaimerChecked = e.target.checked;
    ...
    if (applyBtn) {
      applyBtn.onclick = async () => {
        const input = document.querySelector("#booking-voucher-input");
        const fb = document.querySelector("#voucher-feedback");
        const code = (input?.value || "").trim().toUpperCase();
        if (!code) {
          if (fb) { fb.textContent = "Please enter a voucher code."; fb.style.color = "var(--terra)"; }
          return;
        }

        applyBtn.disabled = true;
        applyBtn.textContent = "Checking...";
        if (fb) { fb.textContent = "Validating voucher..."; fb.style.color = "#64748b"; }

        try {
          const res = await LexAPI.validateVoucher(code, l.id, 45);
          if (res.valid) {
            booking.voucherCode = res.code;
            booking.voucherDiscountPercent = res.discount_percent;
            toast(`${res.discount_percent}% promotional discount applied!`);
            bookingView();
          } else {
            throw new Error(res.message || "Invalid voucher code.");
          }
        } catch (err) {
          if (fb) {
            fb.textContent = err.message || "Invalid or expired promotional code.";
            fb.style.color = "var(--terra)";
          }
        } finally {
    ...
    const removeBtn = document.querySelector("#btn-remove-voucher");
    if (removeBtn) {
      removeBtn.onclick = () => {
        delete booking.voucherCode;
        delete booking.voucherDiscountPercent;
      ...
      content.innerHTML = `
        <div class="success">
          ...
          <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-bottom:12px;">
            ...
            ${booking.id ? `
              <a href="${LexAPI.getBookingReceiptUrl(booking.id)}" target="_blank" rel="noopener" class="ghost secondary" style="display:inline-flex;align-items:center;gap:6px;text-decoration:none;padding:8px 16px;border-radius:6px;font-size:14px;">
                🧾 Tax Invoice &amp; Receipt
              </a>
            ` : ""}
          </div>
...
/**
 * Show a phone-collection modal before proceeding to payment.
 * Returns the 10-digit phone string, or null if user skips.
 * Saves to backend and caches in sessionStorage so we only ask once per session.
 */
async function collectPhone() {
  // Already collected this session
  if (sessionStorage.getItem("vm_phone_confirmed")) return true;

  // Check if user already has phone on backend
  try {
    const me = await LexAPI.request("/auth/me", { method: "GET" });
    if (me.phone_set) {
      sessionStorage.setItem("vm_phone_confirmed", "1");
      return true;
    }
  } catch (_) { /* proceed to show modal */ }
  ...
  return new Promise((resolve) => {
    ...
    overlay.style.cssText = `
      ...
      background:rgba(10,20,15,0.72);backdrop-filter:blur(6px);animation:fadeIn .2s ease;
    `;
    overlay.innerHTML = `
      <div style="
        background:#fff;border-radius:20px;padding:36px 32px;max-width:400px;width:90%;
        box-shadow:0 24px 60px rgba(0,0,0,.22);position:relative;text-align:center;
        animation:slideUp .25s cubic-bezier(.34,1.56,.64,1);
      ">
        <div style="width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#173c30,#2a6b4f);
          display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:22px;">📱</div>
        <h3 style="margin:0 0 6px;font-size:20px;color:#111;font-weight:700;">Add your mobile number</h3>
        <p style="margin:0 0 22px;font-size:14px;color:#666;line-height:1.5;">
          Required to receive payment updates and consultation reminders via SMS.
        </p>
        <div style="position:relative;margin-bottom:8px;">
          <span style="position:absolute;left:14px;top:50%;transform:translateY(-50%);font-size:14px;color:#444;font-weight:600;">+91</span>
          <input id="phone-modal-input" type="tel" maxlength="10" inputmode="numeric" placeholder="98765 43210"
            style="width:100%;box-sizing:border-box;padding:13px 14px 13px 46px;border:1.5px solid #ddd;border-radius:10px;
            font-size:16px;outline:none;transition:border .2s;letter-spacing:1px;"
            oninput="this.value=this.value.replace(/\\D/g,'');document.getElementById('phone-modal-err').textContent='';"
            onfocus="this.style.borderColor='#173c30'" onblur="this.style.borderColor='#ddd'"
          />
        </div>
        <p id="phone-modal-err" style="color:#c0392b;font-size:12px;min-height:16px;margin:0 0 16px;text-align:left;padding-left:4px;"></p>
        <button id="phone-modal-save" style="
          width:100%;padding:14px;background:linear-gradient(135deg,#173c30,#2a6b4f);color:#fff;
        ...
        <button id="phone-modal-skip" style="
          width:100%;padding:10px;background:none;border:none;color:#999;font-size:13px;cursor:pointer;
        ">Skip for now</button>
        <p style="margin:14px 0 0;font-size:11px;color:#aaa;">
          🔒 Stored securely. Not shared with lawyers.
        </p>
      </div>
    `;

    // Inject animation keyframes once
    if (!document.getElementById("phone-modal-style")) {
      const s = document.createElement("style");
      s.id = "phone-modal-style";
      s.textContent = `
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        @keyframes slideUp{from{transform:translateY(30px);opacity:0}to{transform:translateY(0);opacity:1}}
        #phone-modal-save:hover{opacity:.88}
      `;
    ...
    // Focus input
    setTimeout(() => document.getElementById("phone-modal-input")?.focus(), 100);

    const cleanup = () => overlay.remove();

    document.getElementById("phone-modal-save").onclick = async () => {
      const input = document.getElementById("phone-modal-input");
      const errEl = document.getElementById("phone-modal-err");
      ...
      if (!"6789".includes(digits[0])) {
        errEl.textContent = "Number must start with 6, 7, 8, or 9.";
        input.focus(); return;
      }

      const btn = document.getElementById("phone-modal-save");
      ...
      try {
        ...
        errEl.textContent = e.message || "Could not save phone. Please try again.";
        btn.disabled = false; btn.textContent = "Continue to Payment →";
      }
    };

    document.getElementById("phone-modal-skip").onclick = () => {
      cleanup();
      resolve(true); // Allow checkout even if skipped
    };

    // Enter key submits
    document.getElementById("phone-modal-input").onkeydown = (e) => {
      if (e.key === "Enter") document.getElementById("phone-modal-save").click();
    };
  });
}
...
async function handlePay() {
  ...
  try {
    ...
    // Collect phone before checkout if not already saved
    await collectPhone();
    ...
    const payload = {
      ...
      voucher_code: booking.voucherCode || undefined
    };

    const res = await LexAPI.createBooking(payload);

    if (res.status === "confirmed" && res.amount_minor === 0) {
      booking.id = res.id;
...
async function performGoogleAuth(role = "client") {
  if (window.LexAPI && typeof LexAPI.performGoogleAuth === "function") {
    return LexAPI.performGoogleAuth(role);
  }
  if (typeof window.performGoogleAuth === "function") {
    return window.performGoogleAuth(role);
  }
...
document.addEventListener("change", async e => {
  ...
  try {
    ...
    if (method === "PUT") {
      ...
      if (presign.upload.headers) Object.assign(headers, presign.upload.headers);
    } else {
    ...
    const s3 = await fetch(uploadUrl, { method, body: bodyData, headers });
              ...
              <div class="card-dots-wrapper">
                ...
                <div class="card-dots-menu" id="card-menu-${b.id}" hidden>
                  ...
                  ${["confirmed", "in_progress", "completed"].includes(b.status) ? `
                    <div class="card-menu-divider"></div>
                    <a href="${LexAPI.getBookingReceiptUrl(b.id)}" target="_blank" rel="noopener" class="card-menu-item">
                      <span class="menu-icon">🧾</span> Tax Invoice &amp; Receipt
                    </a>
                  ` : ""}
    ...
    fileInput.onchange = async () => {
      ...
      for (const file of files) {
        ...
        try {
          ...
          const uploadUrl = LexAPI.resolveUploadUrl(presign.upload.url);
          const res = await fetch(uploadUrl, { method, body: bodyData, headers });
