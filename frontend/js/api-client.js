const LexAPI = (() => {
  ...
  return {
    ...
    async performGoogleAuth(role = "client") {
      ...
      // 0. Ensure Google Identity Services SDK is ready (poll up to 3.5s)
      ...
      // 1. Interactive Google OAuth2 Popup client (recommended flow on button click)
      if (window.google && window.google.accounts && window.google.accounts.oauth2) {
        try {
      ...
      // 1b. If we have access_token (ya29.), fetch user profile from Google to retrieve verified email and name
...
// ── Enterprise Inactivity & Auto-Logout Manager (DPDP Act & Legal Privilege) ──
(() => {
  ...
  // Role-based timeout limits: idle warning + 2-minute countdown
  const ROLE_LIMITS = {
    admin:   { idleMs: 13 * 60 * 1000, countdownMs: 2 * 60 * 1000 }, // 15 mins total
    lawyer:  { idleMs: 43 * 60 * 1000, countdownMs: 2 * 60 * 1000 }, // 45 mins total
    client:  { idleMs: 28 * 60 * 1000, countdownMs: 2 * 60 * 1000 }, // 30 mins total
    default: { idleMs: 28 * 60 * 1000, countdownMs: 2 * 60 * 1000 }  // 30 mins total
  };
  ...
  try {
    ...
    // Fallback handled via storage event
  }
  ...
  function isCallImmune() {
    ...
    // 2. Platform call flag
    if (window.LexCallActive === true) return true;

    // 3. Active video modal or room container in DOM
  ...
  // User input activity tracking (throttled)
  ...
  function checkInactivity() {
    ...
    // Sync latest from storage in case another tab updated it
