...
function validateWizardStep(stepNum) {
  if (stepNum === 1) {
    ...
    const city = ($("#prof-city")?.value || "").trim();
    const addr = ($("#prof-address")?.value || "").trim();
  ...
  if (stepNum === 2) {
    ...
    const enrollDate = $("#prof-enrollment")?.value;
  ...
  if (stepNum === 3) {
    const aadhaar = ($("#aadhaar-number")?.value || "").trim();
    const mobile = ($("#prof-mobile")?.value || "").trim();
    if (!aadhaar || !/^\d{12}$|^\d{4}-\d{4}-\d{4}$/.test(aadhaar)) {
      toast("Please enter a valid 12-digit Aadhaar number for KYC.");
      $("#aadhaar-number")?.focus();
      return false;
    }
