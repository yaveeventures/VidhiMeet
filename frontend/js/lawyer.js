...
function openUpiVerifyModal() {
  ...
  if (confirmBtn) {
    ...
    confirmBtn.textContent = "✓ Confirm & Verify Bank Account";
  }

  // Display bank info
  if (info) {
    if (bankAccount && bankAccount.account_number_masked) {
      ...
      info.innerHTML = `
        ...
        <p style="margin:4px 0 0;font-size:13px;">Holder: <strong>${escapeHtml(bankAccount.account_holder_name || 'Advocate')}</strong></p>
...
async function confirmBankVerification() {
  ...
  try {
    const res = await LexAPI.initiateUpiVerification();
    closeUpiVerifyModal();
    bankAccount = await LexAPI.getBankAccount().catch(() => null);
    renderBankAccount();
    toast(res.message || "Bank account verified successfully ✓");
  } catch (err) {
    if (errEl) {
      errEl.textContent = err.message || "Failed to verify bank account.";
    }
    toast("Verification failed: " + (err.message || "Unknown error"));
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.textContent = "✓ Confirm & Verify Bank Account";
    }
  }
}
...
(function initBankModal() {
  ...
  if (verifyConfirm) verifyConfirm.onclick = confirmBankVerification;
