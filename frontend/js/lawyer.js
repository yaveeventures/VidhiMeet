...
function checkLawyerSession() {
  ...
  if (!user || user.role !== "lawyer") {
    ...
    document.documentElement.classList.add("lawyer-not-authenticated");
    const sidebar = $("#sidebar");
    const app = $(".app");
    const authPage = $("#lawyer-auth-page");
  ...
  document.documentElement.classList.remove("lawyer-not-authenticated");
  const sidebar = $("#sidebar");
  const app = $(".app");
  const authPage = $("#lawyer-auth-page");
...
function initLawyerAuth() {
  ...
  // Handle #login and #register routing
  function applyAuthHash() {
    ...
    if (hash === "#register") {
      ...
      setTimeout(() => {
        const emailInput = document.getElementById("lawyer-login-email");
...
// ── Onboarding & Profile Completeness Engine ────────────────────────────────
function calculateProfileCompleteness() {
  let score = 0;
  const breakdown = [];

  // 1. Basic details & practice areas (15%)
  const hasName = Boolean(lawyerProfile.full_name && lawyerProfile.full_name.trim());
  const practices = lawyerProfile.practice;
  const hasPractice = Boolean(practices && (Array.isArray(practices) ? practices.length > 0 : String(practices).trim().length > 0));
  const basicOk = hasName && hasPractice;
  if (basicOk) score += 15;
  breakdown.push({ key: "basic", label: "Basic details & practice areas", complete: basicOk, weight: 15 });

  // 2. Bar Council Registration (20%)
  const hasBar = Boolean(lawyerProfile.bar_number && !lawyerProfile.bar_number.startsWith("PENDING") && lawyerProfile.bar_number.trim());
  if (hasBar) score += 20;
  breakdown.push({ key: "bar", label: "State Bar Council enrollment", complete: hasBar, weight: 20 });

  // 3. Experience & Address (15%)
  const hasAddress = Boolean(lawyerProfile.practice_address && lawyerProfile.practice_address.trim().length >= 5);
  const hasEnrollment = Boolean(lawyerProfile.enrollment_date);
  const expOk = hasAddress && hasEnrollment;
  if (expOk) score += 15;
  breakdown.push({ key: "experience", label: "Enrollment date & practice address", complete: expOk, weight: 15 });

  // 4. Bar Certificate Document (20%)
  const hasBarDoc = Boolean(lawyerProfile.bar_license_url);
  const isBarVerified = Boolean(lawyerProfile.verified || lawyerProfile.bar_license_verified);
  if (hasBarDoc) score += 20;
  breakdown.push({ key: "bar_doc", label: "Bar Council Certificate", complete: hasBarDoc, verified: isBarVerified, weight: 20 });

  // 5. Aadhaar Government ID (15%)
  const hasAadhaarDoc = Boolean(lawyerProfile.aadhaar_url);
  const isAadhaarVerified = Boolean(lawyerProfile.verified || lawyerProfile.aadhaar_verified);
  if (hasAadhaarDoc) score += 15;
  breakdown.push({ key: "id_doc", label: "Aadhaar Identity Document", complete: hasAadhaarDoc, verified: isAadhaarVerified, weight: 15 });

  // 6. Mobile number (10%)
  const mobileOk = Boolean(lawyerProfile.mobile_number && /^[6-9][0-9]{9}$/.test(lawyerProfile.mobile_number));
  if (mobileOk) score += 10;
  breakdown.push({ key: "mobile", label: "Mobile number provided", complete: mobileOk, weight: 10 });

  // 7. Payout Bank Account (5%)
  const bankOk = Boolean(bankAccount && (bankAccount.account_number_masked || bankAccount.account_number));
  if (bankOk) score += 5;
  breakdown.push({ key: "bank", label: "Payout bank account", complete: bankOk, weight: 5 });

  return { score: Math.min(score, 100), breakdown };
}

let barCompoundInitialized = false;
function initBarCouncilCompound() {
  if (barCompoundInitialized) return;
  const stateSelect = document.getElementById("prof-bar-state");
  const numInput = document.getElementById("prof-bar-num");
  const yearInput = document.getElementById("prof-bar-year");
  const masterInput = document.getElementById("prof-bar");
  const previewEl = document.getElementById("prof-bar-preview");
  const toggleBtn = document.getElementById("prof-bar-manual-toggle");
  const manualWrap = document.getElementById("prof-bar-manual-wrap");
  const manualInput = document.getElementById("prof-bar-manual");

  if (!masterInput || !stateSelect) return;
  barCompoundInitialized = true;
  ...
  function syncToMaster() {
    ...
    if (stateVal && stateVal !== "OTHER") {
      const prefix = stateVal.endsWith("/") ? stateVal : stateVal + "/";
      const formatted = `${prefix}${num}${year ? '/' + year : ''}`;
      masterInput.value = formatted;
      if (previewEl) previewEl.textContent = formatted;
    } else if (num || year) {
      const formatted = `${num}${year ? '/' + year : ''}`;
      masterInput.value = formatted;
      if (previewEl) previewEl.textContent = formatted;
    } else {
      masterInput.value = "";
  ...
  [stateSelect, numInput, yearInput].forEach(el => {
    if (el) {
      el.addEventListener("input", syncToMaster);
      el.addEventListener("change", syncToMaster);
    }
  });

  if (manualInput) {
    manualInput.addEventListener("input", syncToMaster);
  }
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener("click", () => {
      if (manualWrap.style.display === "none") {
        ...
        toggleBtn.textContent = "Guided inputs";
        if (manualInput) {
          manualInput.value = masterInput.value;
          manualInput.focus();
        }
      } else {
        manualWrap.style.display = "none";
        toggleBtn.textContent = "Edit manually";
        syncToMaster();
      }
    });
  }
...
function parseAndSetBarCouncilNumber(barNumber) {
  initBarCouncilCompound();
  const stateSelect = document.getElementById("prof-bar-state");
  const numInput = document.getElementById("prof-bar-num");
  const yearInput = document.getElementById("prof-bar-year");
  const masterInput = document.getElementById("prof-bar");
  ...
  // Ignore provisional backend placeholders (e.g. PENDING-...)
  if (!barNumber || barNumber.startsWith("PENDING")) {
    masterInput.value = "";
    if (previewEl) previewEl.textContent = "—";
    ...
    if (manualWrap) manualWrap.style.display = "none";
    if (toggleBtn) toggleBtn.textContent = "Edit manually";
    return;
  }

  masterInput.value = barNumber;
  if (previewEl) previewEl.textContent = barNumber;
  if (manualInput) manualInput.value = barNumber;

  const parts = barNumber.split("/").map(p => p.trim());
  let matched = false;

  if (parts.length >= 2 && stateSelect) {
    const rawPrefix = parts[0].toUpperCase();
    const prefixWithSlash = rawPrefix + "/";
    for (const opt of stateSelect.options) {
      if (opt.value === prefixWithSlash || (opt.value && opt.value.replace('/', '') === rawPrefix)) {
    ...
    if (matched) {
      if (parts.length === 3) {
        if (/^[1-2][0-9]{3}$/.test(parts[1])) {
          if (yearInput) yearInput.value = parts[1];
          if (numInput) numInput.value = parts[2];
        } else {
          if (numInput) numInput.value = parts[1];
          if (yearInput) yearInput.value = parts[2];
        }
  ...
  if (toggleBtn) toggleBtn.textContent = "Guided inputs";
}

function checkWelcomeModal() {
  const seen = localStorage.getItem("vidhimeet_lawyer_welcome_seen");
  if (!seen && !lawyerProfile.verified) {
    const welcomeModal = document.getElementById("lawyer-welcome-modal");
    if (welcomeModal) {
...
let onboardingModalsInitialized = false;
function initOnboardingModals() {
  if (onboardingModalsInitialized) return;
  onboardingModalsInitialized = true;

  // Welcome modal buttons
  const welcomeModal = document.getElementById("lawyer-welcome-modal");
  const welcomeClose = document.getElementById("lawyer-welcome-close");
  ...
  const closeWelcome = () => {
    localStorage.setItem("vidhimeet_lawyer_welcome_seen", "1");
    if (welcomeModal) welcomeModal.hidden = true;
  };

  if (welcomeClose) welcomeClose.onclick = closeWelcome;
  if (welcomeDismiss) welcomeDismiss.onclick = closeWelcome;
  if (welcomeStart) {
  ...
  // SLA modal buttons
  const slaModal = document.getElementById("review-sla-modal");
  const slaClose = document.getElementById("review-sla-close");
  const slaDone = document.getElementById("review-sla-done-btn");

  const closeSla = () => {
    if (slaModal) slaModal.hidden = true;
    view("overview");
  };

  if (slaClose) slaClose.onclick = closeSla;
  if (slaDone) slaDone.onclick = closeSla;
  if (slaModal) {
    slaModal.addEventListener("click", e => {
      if (e.target === slaModal) closeSla();
    });
  }
}
...
function renderOverview() {
  ...
  const overviewVerificationContainer = document.getElementById("overview-verification-container") || $(".verified");
  if (overviewVerificationContainer) {
    if (lawyerProfile.verified) {
      const nextYear = new Date().getFullYear() + 1;
      overviewVerificationContainer.innerHTML = `
        <div class="verified">
          <i>✓</i>
          <div>
            <strong>Your advocate profile is active & verified</strong>
            <small>Credentials verified by VidhiMeet Compliance Desk. Next annual review: 12 May ${nextYear}.</small>
          </div>
          <button data-go="profile">View credentials →</button>
        </div>
      `;
    } else {
      ...
      const mobileOk = Boolean(lawyerProfile.mobile_number && /^[6-9][0-9]{9}$/.test(lawyerProfile.mobile_number));
      const bankOk = Boolean(bankAccount && (bankAccount.account_number_masked || bankAccount.account_number));
      const basicOk = Boolean(lawyerProfile.full_name && lawyerProfile.practice && (Array.isArray(lawyerProfile.practice) ? lawyerProfile.practice.length > 0 : true) && lawyerProfile.practice_address && lawyerProfile.enrollment_date);

      const step1Status = basicOk ? `<span class="step-status-pill completed">Completed ✓</span>` : `<span class="step-status-pill pending">Pending</span>`;
      const step1Btn = `<button class="outline" style="padding:4px 10px;font-size:12px;" data-go="profile" data-step="1">${basicOk ? 'Edit profile' : 'Complete details →'}</button>`;

      const step2Status = (hasBarDoc && isBarVerified) ? `<span class="step-status-pill completed">Verified ✓</span>` : (hasBarDoc ? `<span class="step-status-pill pending">In Review</span>` : `<span class="step-status-pill missing">Upload Needed</span>`);
      const step2Btn = `<button class="primary" style="padding:4px 10px;font-size:12px;" data-go="profile" data-step="2">${hasBarDoc ? 'View credentials' : 'Upload certificate →'}</button>`;

      const step3Status = (hasAadhaarDoc && isAadhaarVerified) ? `<span class="step-status-pill completed">Verified ✓</span>` : (hasAadhaarDoc ? `<span class="step-status-pill pending">In Review</span>` : `<span class="step-status-pill missing">Action Needed</span>`);
      const step3Btn = `<button class="primary" style="padding:4px 10px;font-size:12px;" data-go="profile" data-step="3">${hasAadhaarDoc ? 'View KYC' : 'Verify identity →'}</button>`;

      const step4Status = bankOk ? `<span class="step-status-pill completed">Linked ✓</span>` : `<span class="step-status-pill missing">Not Linked</span>`;
      const step4Btn = `<button class="outline" style="padding:4px 10px;font-size:12px;" data-go="profile" data-step="3">${bankOk ? 'Manage account' : 'Add bank account →'}</button>`;

      const isRejected = lawyerProfile.verification_status === "rejected";
      const rejectionBanner = isRejected ? `
        <div style="background:#fff5f5; border:1px solid #feb2b2; border-left:5px solid #e53e3e; border-radius:8px; padding:14px 18px; margin-bottom:16px;">
          <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap;">
            <div style="flex:1; min-width:240px;">
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
                <span style="font-size:18px;">⚠️</span>
                <strong style="color:#c53030; font-size:15px;">Credentials Revision Requested</strong>
              </div>
              <p style="margin:4px 0 8px; font-size:13px; color:#2d3748; line-height:1.5;">
                Your application was reviewed by the VidhiMeet Compliance Desk and requires correction before activation:
              </p>
              <div style="background:#fff; border:1px solid #fed7d7; border-radius:6px; padding:10px 14px; font-size:13px; color:#9b2c2c; font-weight:600;">
                "${escapeHtml(lawyerProfile.rejection_reason || "Credentials did not meet compliance requirements")}"
              </div>
              <p style="margin:8px 0 0; font-size:12px; color:#718096;">
                Please re-upload clear credentials below. Your profile will be reviewed promptly upon update.
              </p>
            </div>
            <button class="primary" style="background:#c53030; border-color:#c53030; white-space:nowrap; padding:8px 14px; font-size:12px; margin-top:4px;" data-go="profile" data-step="2">
              Update Credentials →
            </button>
          </div>
        </div>
      ...
      overviewVerificationContainer.innerHTML = `
        ...
        <div class="onboarding-checklist-card">
          <div class="onboarding-head">
            <div>
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
                ...
                <h3 style="margin:0;font-size:18px;color:var(--dark);">Advocate Onboarding & Activation Hub</h3>
              </div>
              <p style="margin:0;font-size:13px;color:var(--muted);">Complete the steps below to activate your digital practice for video consultations and drafting assignments.</p>
            </div>
            <div class="onboarding-progress-badge">
              <span>Completeness: <strong>${score}%</strong></span>
              <div class="progress" style="width:100px;height:8px;margin:0;background:#e2dacd;"><i style="width:${score}%;background:var(--forest);"></i></div>
            </div>
          </div>
          <div class="onboarding-steps-grid">
            <div class="onboarding-step-item">
              <div class="onboarding-step-top">
                ...
                <div class="onboarding-step-info">
                  <strong>1. Practice & Experience</strong>
                  <small>Specializations, practice address, and consultation fee.</small>
                </div>
              </div>
            ...
            <div class="onboarding-step-item">
              <div class="onboarding-step-top">
                ...
                <div class="onboarding-step-info">
                  <strong>2. Bar Council Credential</strong>
                  <small>State Bar enrollment number & Bar Certificate document.</small>
                </div>
              </div>
              <div class="onboarding-step-action">
                ${step2Status}
            ...
            <div class="onboarding-step-item">
              <div class="onboarding-step-top">
                <span class="onboarding-step-icon">🪪</span>
                <div class="onboarding-step-info">
                  <strong>3. Identity & KYC Verification</strong>
                  <small>Government photo ID (Aadhaar) & verified mobile contact.</small>
                </div>
              </div>
              <div class="onboarding-step-action">
                ${step3Status}
            ...
            <div class="onboarding-step-item">
              <div class="onboarding-step-top">
                <span class="onboarding-step-icon">🏦</span>
                <div class="onboarding-step-info">
                  <strong>4. Payout Account</strong>
                  <small>Encrypted bank details & IFSC for consultation earnings.</small>
                </div>
              </div>
              <div class="onboarding-step-action">
                ${step4Status}
          ...
          <div class="onboarding-sla-bar">
            <div style="display:flex;align-items:center;gap:8px;">
              <span>🛡️</span>
              <span>Administrative Compliance Review SLA: <strong>12–24 Business Hours</strong></span>
            </div>
            <div style="display:flex;align-items:center;gap:12px;">
              <span style="color:var(--muted);">Need help?</span>
              <a href="#" onclick="openSupportModal(); return false;" style="color:var(--forest);font-weight:600;text-decoration:underline;">Advocate Desk Support →</a>
            </div>
          </div>
        </div>
      `;

      overviewVerificationContainer.querySelectorAll("button[data-go]").forEach(btn => {
        btn.addEventListener("click", () => {
          const target = btn.dataset.go;
          const targetStep = btn.dataset.step;
          if (target) {
            view(target);
            if (targetStep && typeof goToWizardStep === "function") {
              setTimeout(() => goToWizardStep(parseInt(targetStep), true), 80);
            }
          }
        });
      });
...
function renderBankAccount() {
  ...
  if (!bankAccount) {
    display.innerHTML = `
      <div class="empty-bank-box">
        ...
        <p class="empty-bank-text">
          Link your bank account to receive direct weekly settlements for completed consultations and drafting milestones.
        </p>
        <div class="empty-bank-actions">
          <button type="button" class="btn-primary-link-rpd" id="auto-link-rpd-btn">
            ⚡ Auto-Link &amp; Verify via UPI (Instant ₹1)
          </button>
          <button type="button" class="btn-manual-link" id="add-bank-btn">
    ...
    const autoLinkBtn = document.getElementById("auto-link-rpd-btn");
    if (autoLinkBtn) autoLinkBtn.onclick = () => openUpiVerifyModal();
  ...
  const verifiedBadge = bankAccount.verified
    ? `<span class="bank-account-badge verified"><span class="badge-dot">●</span> Verified Account</span>`
    : `<span class="bank-account-badge unverified"><span class="badge-dot">●</span> Unverified</span>`;

  const vpaRow = bankAccount.upi_vpa
    ? `<div class="bank-data-item">
         <span class="bank-data-label">UPI VPA</span>
         <span class="bank-data-value mono">${escapeHtml(bankAccount.upi_vpa)}</span>
       </div>`
    : '';

  const utrRow = bankAccount.utr
    ? `<div class="bank-data-item">
         <span class="bank-data-label">Verification UTR</span>
         <span class="bank-data-value mono">${escapeHtml(bankAccount.utr)}</span>
       </div>`
    : '';

  const verifyCallout = bankAccount.verified ? '' : `
    <button type="button" class="btn-verify-upi-modern" id="upi-verify-open-btn">
      <span class="verify-icon">⚡</span> Auto-Verify via UPI Reverse Penny Drop
    </button>`;

  display.innerHTML = `
    <div class="bank-card-premium">
      <div class="bank-card-top">
        <div class="bank-brand-wrap">
          <div class="bank-emblem">🏦</div>
          <div>
            <div class="bank-name-title">${escapeHtml(bankAccount.bank_name || "Primary Payout Bank")}</div>
            <div class="bank-holder-name">${escapeHtml(bankAccount.account_holder_name || "Advocate")}</div>
          </div>
        </div>
        ${verifiedBadge}
      </div>
      ...
      <div class="bank-data-grid">
        <div class="bank-data-item">
          <span class="bank-data-label">Account Number</span>
          <span class="bank-data-value mono">${escapeHtml(bankAccount.account_number_masked || "••••••••••••")}</span>
        </div>
        <div class="bank-data-item">
          <span class="bank-data-label">IFSC Code</span>
          <span class="bank-data-value mono">${escapeHtml(bankAccount.ifsc_code || "—")}</span>
        </div>
        ${vpaRow}
        ${utrRow}
      </div>
    ...
    <div class="payout-meta-list">
      <div class="payout-meta-row">
        <span class="payout-meta-label">📅 Payout Frequency</span>
        <span class="payout-meta-val">Weekly (Every Friday)</span>
      </div>
      <div class="payout-meta-row">
        <span class="payout-meta-label">🗓 Next Settlement Date</span>
        <span class="payout-meta-val" id="payout-next-date">—</span>
      </div>
      <div class="payout-meta-row">
        <span class="payout-meta-label">💳 Platform Retainage</span>
        <span class="payout-meta-val">5% (95% Lawyer Net)</span>
      </div>
    </div>

    <div class="payout-legal-note">
      <span style="font-size:14px;line-height:1;">ℹ</span>
      <span>Direct bank deposits are automatically disbursed to this verified account following the 7-day consultation dispute window or immediate draft milestone approval.</span>
    </div>
  `;

  if (editBtn) { 
    editBtn.hidden = false; 
    editBtn.onclick = () => openBankModal(true); 
  }
  if (delBtn)  { 
    delBtn.hidden  = false; 
    delBtn.onclick  = confirmDeleteBankAccount; 
  }

  const verifyOpenBtn = document.getElementById("upi-verify-open-btn");
  if (verifyOpenBtn) verifyOpenBtn.onclick = openUpiVerifyModal;

  // Payout next-Friday calculation
  const payoutDateEl = document.getElementById("payout-next-date");
  if (payoutDateEl) {
    const d = new Date(); 
    const daysUntilFriday = (5 - d.getDay() + 7) % 7 || 7;
    d.setDate(d.getDate() + daysUntilFriday);
    payoutDateEl.textContent = d.toLocaleDateString("en-IN", {day: "numeric", month: "long", year: "numeric"});
  }

  // PAN warning banner — show if PAN is missing
  ...
  if (!hasPan) {
    if (!panWarning) {
      ...
      panWarning.style.cssText = "margin-top:14px;padding:12px 16px;background:#fff7ed;border:1px solid #fdba74;border-radius:12px;font-size:12.5px;color:#9a3412;display:flex;align-items:flex-start;gap:10px;line-height:1.45;";
      panWarning.innerHTML = `<span style="font-size:18px;flex-shrink:0;">⚠️</span><span><strong>PAN missing — payouts held in escrow.</strong> Add your PAN number in <a href="#profile" style="color:#c2410c;font-weight:700;text-decoration:underline;">Profile (KYC)</a> or click Edit above to enable automatic disbursement.</span>`;
...
async function handleBankFormSubmit(e) {
  ...
  const acnoEl  = document.getElementById("bank-acno");
  ...
  if (!holder || !bName || !ifsc) {
    errEl.textContent = "All required fields must be filled.";
    if (!holder && VR) VR.markFieldInvalid(holderEl);
    else if (!bName && VR) VR.markFieldInvalid(bNameEl);
    else if (!ifsc && VR) VR.markFieldInvalid(ifscEl);
    return;
  }

  const isIfscValid = VR ? VR.isValidIFSC(ifsc) : /^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifsc);
  if (!isIfscValid) {
    errEl.textContent = "IFSC must be 11 characters (e.g. HDFC0001234).";
    if (VR) VR.markFieldInvalid(ifscEl);
    else ifscEl?.focus();
    return;
  }
  ...
  if (!isEdit || acno) {
    if (!acno) {
      errEl.textContent = "Account number is required.";
      if (VR) VR.markFieldInvalid(acnoEl);
      else acnoEl?.focus();
      return;
    }
    const isAcnoValid = VR ? VR.isValidAccountNumber(acno) : /^\d{6,18}$/.test(acno);
    if (!isAcnoValid) {
      errEl.textContent = "Account number must be 6–18 digits.";
      if (VR) VR.markFieldInvalid(acnoEl);
      else acnoEl?.focus();
      return;
    }
    if (acno !== acnoC) {
      errEl.textContent = "Account numbers do not match.";
      if (VR) VR.markFieldInvalid(acnoCEl);
      else acnoCEl?.focus();
      return;
    }
  }
  ...
  if (vpa) {
    const isVpaValid = VR ? VR.isValidVpa(vpa) : /@/.test(vpa);
    if (!isVpaValid) {
      errEl.textContent = "Invalid UPI VPA format (e.g. name@okhdfcbank).";
      if (VR) VR.markFieldInvalid(vpaEl);
      else vpaEl?.focus();
      return;
    }
  }
...
let activeRpdVerificationId = null;
let rpdPollTimer = null;
let rpdCountdownTimer = null;
let rpdExpiresAtSeconds = 0;

function clearRpdTimers() {
  if (rpdPollTimer) {
    clearInterval(rpdPollTimer);
    rpdPollTimer = null;
  }
  if (rpdCountdownTimer) {
    clearInterval(rpdCountdownTimer);
    rpdCountdownTimer = null;
  }
}

function closeUpiVerifyModal() {
  clearRpdTimers();
  activeRpdVerificationId = null;
...
function openUpiVerifyModal() {
  ...
  const initView    = document.getElementById("rpd-init-view");
  const activeView  = document.getElementById("rpd-active-view");
  const successView = document.getElementById("rpd-success-view");
  const confirmBtn  = document.getElementById("upi-verify-confirm-btn");
  ...
  if (confirmBtn) {
    ...
    confirmBtn.textContent = "⚡ Generate ₹1 UPI QR Code →";
  }

  // Display bank info if user already has an account entered
  if (info) {
    if (bankAccount && bankAccount.account_number_masked && bankAccount.account_number_masked !== "PENDING") {
      info.style.display = "block";
      info.innerHTML = `
        <p style="margin:0 0 6px;font-size:13px;"><strong>${escapeHtml(bankAccount.bank_name || 'Bank Account')}</strong></p>
  ...
  // Auto-initiate the RPD session immediately for a seamless UX
  initiateRpdVerification();
}

function updateRpdCountdown() {
  const timerEl = document.getElementById("rpd-timer");
  if (!timerEl) return;
  const now = Math.floor(Date.now() / 1000);
  const remaining = Math.max(0, rpdExpiresAtSeconds - now);
  const mins = Math.floor(remaining / 60);
  const secs = remaining % 60;
  timerEl.textContent = `Valid: ${mins}:${secs < 10 ? '0' : ''}${secs}`;

  if (remaining <= 0) {
    clearRpdTimers();
    const errEl = document.getElementById("upi-verify-error");
    if (errEl) errEl.textContent = "QR code expired. Please generate a new one.";
    const activeView = document.getElementById("rpd-active-view");
    const initView = document.getElementById("rpd-init-view");
    if (activeView) activeView.style.display = "none";
    if (initView) initView.style.display = "block";
...
async function initiateRpdVerification() {
  const btn         = document.getElementById("upi-verify-confirm-btn");
  const errEl       = document.getElementById("upi-verify-error");
  const initView    = document.getElementById("rpd-init-view");
  const activeView  = document.getElementById("rpd-active-view");
  const qrImg       = document.getElementById("rpd-qr-image");
  const intentLink  = document.getElementById("rpd-intent-gpay");
  const simulateBox = document.getElementById("rpd-simulate-box");
  const statusText  = document.getElementById("rpd-status-text");
  ...
  try {
    const res = await LexAPI.initiateReversePennyDrop();
    if (!res || !res.verification_id) {
      throw new Error(res?.message || "Failed to initiate UPI verification.");
    }

    activeRpdVerificationId = res.verification_id;

    if (res.qr_code && qrImg) {
      qrImg.src = res.qr_code;
    }

    // Configure UPI Intent URL for mobile
    let upiUrl = "";
    if (typeof res.upi_intent === "object" && res.upi_intent !== null) {
      upiUrl = res.upi_intent.upi_uri || res.upi_intent.gpay_link || res.payment_link || "";
    } else if (typeof res.upi_intent === "string") {
      upiUrl = res.upi_intent;
    } else if (res.payment_link) {
      upiUrl = res.payment_link;
    }

    if (intentLink) {
      if (upiUrl) {
        intentLink.href = upiUrl;
        intentLink.style.display = "block";
      } else {
        intentLink.style.display = "none";
      }
    }

    // Set expiration timer (10 mins fallback or from server)
    const validSeconds = res.valid_upto ? Math.max(0, res.valid_upto - Math.floor(Date.now() / 1000)) : 600;
    rpdExpiresAtSeconds = Math.floor(Date.now() / 1000) + (validSeconds > 0 ? validSeconds : 600);
    updateRpdCountdown();
    if (rpdCountdownTimer) clearInterval(rpdCountdownTimer);
    rpdCountdownTimer = setInterval(updateRpdCountdown, 1000);

    // Show mock simulation button if in test/mock mode
    if (simulateBox) {
      simulateBox.style.display = res.is_mock ? "block" : "none";
    }

    if (statusText) {
      statusText.textContent = "Listening for ₹1 payment confirmation...";
    }
    ...
    const message = err.message || "Failed to initiate UPI verification.";
    if (errEl) {
      errEl.innerHTML = `
        <div style="margin-bottom:10px;line-height:1.4;">${escapeHtml(message)}</div>
        <button type="button" id="rpd-err-manual-btn" style="width:100%;padding:10px;border-radius:10px;font-size:13px;font-weight:600;background:#f3f4f6;color:var(--dark);border:1px solid var(--line);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;">
          ＋ Enter Bank Account Details Manually
        </button>
      `;
      const fallbackBtn = document.getElementById("rpd-err-manual-btn");
      if (fallbackBtn) {
        fallbackBtn.onclick = () => {
          closeUpiVerifyModal();
          openBankModal(Boolean(bankAccount));
        };
      }
    }
    if (initView) initView.style.display = "block";
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = "⚡ Generate ₹1 UPI QR Code →";
    }
  }
}

function startRpdPolling() {
  if (rpdPollTimer) clearInterval(rpdPollTimer);
  rpdPollTimer = setInterval(() => {
    checkRpdStatus(false);
  }, 3000);
}

async function checkRpdStatus(manual = false) {
  if (!activeRpdVerificationId) return;

  const errEl       = document.getElementById("upi-verify-error");
  const activeView  = document.getElementById("rpd-active-view");
  const successView = document.getElementById("rpd-success-view");
  const detailsEl   = document.getElementById("rpd-success-details");
  const checkBtn    = document.getElementById("rpd-check-status-btn");
  ...
  try {
    const res = await LexAPI.getReversePennyDropStatus(activeRpdVerificationId);
    if (!res) return;

    if (res.status === "SUCCESS" && res.verified) {
      // Payment received and account verified!
      clearRpdTimers();
      activeRpdVerificationId = null;

      // Refresh lawyer bank account state
      bankAccount = await LexAPI.getBankAccount().catch(() => null);
      renderBankAccount();
      ...
      if (successView) {
        ...
        if (detailsEl) {
          const acctNum = res.account_number_masked || (res.account_number ? `••••${res.account_number.slice(-4)}` : "");
          detailsEl.innerHTML = `
            <strong>${escapeHtml(res.bank_name || (bankAccount && bankAccount.bank_name) || "Bank Account")}</strong><br>
            Account: <strong>${escapeHtml(acctNum || (bankAccount && bankAccount.account_number_masked) || "Verified")}</strong> | IFSC: <strong>${escapeHtml(res.ifsc || (bankAccount && bankAccount.ifsc_code) || "")}</strong><br>
            Name at Bank: <strong>${escapeHtml(res.account_holder_name || (bankAccount && bankAccount.account_holder_name) || "")}</strong><br>
            <span style="display:inline-block;margin-top:6px;font-size:11px;color:var(--forest);">Transaction Ref / UTR: ${escapeHtml(res.utr || "Verified")}</span>
          `;
        }
      }
      toast("Bank account verified successfully via UPI ✓");
    } else if (res.status === "FAILED" || res.status === "EXPIRED") {
      clearRpdTimers();
      if (errEl) errEl.textContent = res.message || "Payment verification failed or expired. Please retry.";
      if (activeView) activeView.style.display = "none";
      const initView = document.getElementById("rpd-init-view");
      if (initView) initView.style.display = "block";
    }
    ...
    if (manual && errEl) {
      errEl.textContent = err.message || "Error checking status.";
    }
  } finally {
    if (manual && checkBtn) {
...
async function simulateRpdPayment() {
  if (!activeRpdVerificationId) return;
  const simBtn = document.getElementById("rpd-simulate-pay-btn");
  const errEl  = document.getElementById("upi-verify-error");
  if (simBtn) {
    simBtn.disabled = true;
    simBtn.textContent = "Simulating ₹1 payment…";
  }
  try {
    await LexAPI.mockCompleteReversePennyDrop({ verification_id: activeRpdVerificationId });
    await checkRpdStatus(false);
  } catch (err) {
    if (errEl) errEl.textContent = "Simulation failed: " + err.message;
  } finally {
    if (simBtn) {
      simBtn.disabled = false;
      simBtn.textContent = "⚡ Simulate ₹1 Payment (Dev Testing)";
    }
  }
}
...
(function initBankModal() {
  ...
  const checkStatusBtn = document.getElementById("rpd-check-status-btn");
  const simPayBtn      = document.getElementById("rpd-simulate-pay-btn");
  const successDoneBtn = document.getElementById("rpd-success-done-btn");

  if (bankForm)       bankForm.onsubmit     = handleBankFormSubmit;
  if (closeBtn)       closeBtn.onclick      = () => { if (bankModal) bankModal.hidden = true; };
  if (verifyClose)    verifyClose.onclick   = closeUpiVerifyModal;
  if (verifyConfirm)  verifyConfirm.onclick = initiateRpdVerification;
  if (checkStatusBtn) checkStatusBtn.onclick = () => checkRpdStatus(true);
  if (simPayBtn)      simPayBtn.onclick      = simulateRpdPayment;
  if (successDoneBtn) successDoneBtn.onclick = closeUpiVerifyModal;

  if (bankModal)   bankModal.addEventListener("click",   e => { if (e.target === bankModal) bankModal.hidden = true; });
  if (verifyModal) verifyModal.addEventListener("click", e => { if (e.target === verifyModal) closeUpiVerifyModal(); });
})();
...
// ── 4-Step Progressive Onboarding Stepper Controller ─────────────────────────
let currentWizardStep = 1;
let wizardInitialized = false;

function goToWizardStep(stepNum, skipValidation = false) {
  stepNum = Math.max(1, Math.min(4, parseInt(stepNum) || 1));
  
  if (!skipValidation && stepNum > currentWizardStep) {
    for (let s = currentWizardStep; s < stepNum; s++) {
  ...
  document.querySelectorAll(".wizard-step-btn").forEach(btn => {
    const s = parseInt(btn.dataset.wizardStep) || 1;
    btn.classList.toggle("active", s === currentWizardStep);
    btn.classList.toggle("completed", s < currentWizardStep);
  });

  // Update Panes
  document.querySelectorAll(".wizard-pane").forEach(pane => {
    const p = parseInt(pane.dataset.pane) || 1;
    pane.classList.toggle("active", p === currentWizardStep);
  });

  // Update main header button label based on step
  const headerSaveBtn = document.getElementById("save-profile-btn");
  if (headerSaveBtn) {
    headerSaveBtn.textContent = currentWizardStep === 4 ? "Submit for Verification →" : "Save Profile";
  }
...
function validateWizardStep(stepNum) {
  ...
  if (stepNum === 1) {
    ...
    const cityEl = $("#prof-city");
    const addrEl = $("#prof-address");
    const name = (nameEl?.value || "").trim();
    const city = (cityEl?.value || "").trim();
    const addr = (addrEl?.value || "").trim();

    if (!name) {
      toast("Please enter your professional name.");
      if (VR) VR.markFieldInvalid(nameEl);
      else nameEl?.focus();
      return false;
    }
    if (!city) {
      toast("Please enter your practice city.");
      if (VR) VR.markFieldInvalid(cityEl);
      else cityEl?.focus();
      return false;
    }
    if (!addr || addr.length < 5) {
      toast("Please enter your complete practice chamber or office address (min 5 characters).");
      if (VR) VR.markFieldInvalid(addrEl);
      else addrEl?.focus();
      return false;
    }
    return true;
  ...
  if (stepNum === 2) {
    const barEl = $("#prof-bar-num") || $("#prof-bar");
    const enrollEl = $("#prof-enrollment");
    const bar = ($("#prof-bar")?.value || "").trim();
    const enrollDate = enrollEl?.value;
    const practiceChecked = document.querySelectorAll("input[name='prof-practice']:checked");

    if (!bar || bar.startsWith("PENDING")) {
      toast("Please provide your State Bar Council Registration Number.");
      if (VR) VR.markFieldInvalid(barEl);
      else barEl?.focus();
      return false;
    }
    if (!enrollDate) {
      toast("Please enter your Date of Enrollment.");
      if (VR) VR.markFieldInvalid(enrollEl);
      else enrollEl?.focus();
      return false;
    }
    if (practiceChecked.length === 0) {
      toast("Please select at least one Primary Practice Area.");
      return false;
    }
    return true;
  }
  ...
  if (stepNum === 3) {
    ...
    const isAadhaarValid = VR ? VR.isValidAadhaar(aadhaar) : /^\d{12}$|^\d{4}-\d{4}-\d{4}$/.test(aadhaar);
    if (!aadhaar || !isAadhaarValid) {
      toast("Please enter a valid 12-digit Aadhaar number for KYC.");
      if (VR) VR.markFieldInvalid(aadhaarEl);
      else aadhaarEl?.focus();
      return false;
    }

    const isMobileValid = VR ? VR.isValidMobile(mobile) : /^[6-9][0-9]{9}$/.test(mobile);
    if (!mobile || !isMobileValid) {
      toast("Please enter a valid 10-digit mobile number.");
      if (VR) VR.markFieldInvalid(mobileEl);
      else mobileEl?.focus();
      return false;
    }
  ...
  if (stepNum === 4) {
    const decl1 = document.getElementById("decl-good-standing");
    const decl2 = document.getElementById("decl-intermediary");
    const decl3 = document.getElementById("decl-confidentiality");

    if (!decl1?.checked || !decl2?.checked || !decl3?.checked) {
      toast("You must accept all three legal declarations to submit your profile.");
      return false;
    }
    return true;
  }
...
function initOnboardingWizard() {
  ...
  document.querySelectorAll(".wizard-step-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const targetStep = parseInt(btn.dataset.wizardStep) || 1;
  ...
  document.querySelectorAll(".wizard-next-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const nextStep = parseInt(btn.dataset.next) || (currentWizardStep + 1);
  ...
  document.querySelectorAll(".wizard-prev-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const prevStep = parseInt(btn.dataset.prev) || (currentWizardStep - 1);
      goToWizardStep(prevStep, true);
    });
  });

  // Court chip toggling style
  document.querySelectorAll(".court-chip input[type='checkbox']").forEach(chk => {
    chk.addEventListener("change", () => {
      const chip = chk.closest(".court-chip");
      if (chip) chip.classList.toggle("selected", chk.checked);
    });
  });

  // Avatar upload trigger
  const avatarBtn = document.getElementById("upload-avatar-trigger");
  const avatarFile = document.getElementById("profile-pic-file");
  if (avatarBtn && avatarFile) {
    ...
    avatarFile.onchange = () => {
      if (avatarFile.files && avatarFile.files[0]) {
        ...
        reader.onload = (e) => {
          const thumb = document.getElementById("avatar-thumbnail");
          const topAv = document.getElementById("prof-avatar-preview");
          if (thumb) thumb.innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
          if (topAv) topAv.innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
          toast("Photo selected. It will be uploaded upon saving.");
        };
  ...
  // Wizard bank button
  const wizardBankBtn = document.getElementById("wizard-bank-btn");
  if (wizardBankBtn) {
    wizardBankBtn.onclick = () => {
      const bankModal = document.getElementById("bank-modal");
  ...
  if (wizardSubmitBtn) {
    wizardSubmitBtn.onclick = handleSaveProfile;
  }
}

// Track whether the user has modified profile form inputs in the wizard
...
// Dynamically refresh profile completeness bar, sidebar checklists, and credential badge without clearing form inputs
function updateProfileCompletenessAndBadges() {
  const { score } = calculateProfileCompleteness();
  const profCompletenessText = $("#prof-completeness-text");
  const profCompletenessBar = $("#prof-completeness-bar");
  if (profCompletenessText) {
    if (lawyerProfile.verified) {
      profCompletenessText.textContent = "Profile verified · 100% complete";
    } else if (lawyerProfile.verification_status === "rejected") {
      profCompletenessText.textContent = "Action required · Credentials revision requested";
      profCompletenessText.style.color = "#c53030";
    } else if (score >= 100) {
      profCompletenessText.textContent = "Profile 100% complete · In compliance review";
    } else {
      profCompletenessText.textContent = `Profile ${score}% complete · Complete remaining steps to activate`;
    }
  }
  if (profCompletenessBar) {
    profCompletenessBar.style.width = (lawyerProfile.verified ? 100 : score) + "%";
  }

  // Legal declarations pre-check if accepted previously
  if (lawyerProfile.availability && lawyerProfile.availability.declarations_accepted_at) {
    const decl1 = document.getElementById("decl-good-standing");
    const decl2 = document.getElementById("decl-intermediary");
    const decl3 = document.getElementById("decl-confidentiality");
    if (decl1) decl1.checked = true;
    if (decl2) decl2.checked = true;
    if (decl3) decl3.checked = true;
  }
  ...
  if (barLicenseStatus) {
    if (isBarVerified) {
      ...
      if (uploadBarBtn) {
        uploadBarBtn.style.display = "inline-block";
        if (lawyerProfile.bar_license_url) uploadBarBtn.textContent = "Change Certificate";
      }
    }
  }
  ...
  if (govIdStatus) {
    if (isAadhaarVerified) {
      ...
      if (uploadIdBtn) {
        uploadIdBtn.style.display = "inline-block";
        if (lawyerProfile.aadhaar_url) uploadIdBtn.textContent = "Change Document";
      }
    }
  }

  // Synchronize right sidebar checklist overview
  const chkBar = $("#sidebar-chk-bar");
  if (chkBar) {
    if (isBarVerified) {
      chkBar.textContent = "Verified ✓";
      chkBar.style.color = "var(--forest)";
    } else if (lawyerProfile.bar_license_url) {
      chkBar.textContent = "Uploaded ✓";
      chkBar.style.color = "var(--forest)";
    } else {
      chkBar.textContent = "Step 2";
      chkBar.style.color = "var(--muted)";
    }
  }

  const chkKyc = $("#sidebar-chk-kyc");
  if (chkKyc) {
    if (isAadhaarVerified) {
      chkKyc.textContent = "Verified ✓";
      chkKyc.style.color = "var(--forest)";
    } else if (lawyerProfile.aadhaar_url) {
      chkKyc.textContent = "Uploaded ✓";
      chkKyc.style.color = "var(--forest)";
    } else {
      chkKyc.textContent = "Step 3";
      chkKyc.style.color = "var(--muted)";
    }
  }

  const chkDecl = $("#sidebar-chk-decl");
  if (chkDecl) {
    const decl1 = document.getElementById("decl-good-standing");
    const decl2 = document.getElementById("decl-intermediary");
    const decl3 = document.getElementById("decl-confidentiality");
    const isDecl = Boolean(decl1?.checked && decl2?.checked && decl3?.checked);
    chkDecl.textContent = isDecl ? "Signed ✓" : "Step 4";
    chkDecl.style.color = isDecl ? "var(--forest)" : "var(--muted)";
  }
}
...
function renderProfile(force = false) {
  ...
  // Attach dirty tracking listener to the profile section if not already attached
  const profileSection = document.getElementById("profile");
  if (profileSection && !profileSection._dirtyTracked) {
    profileSection._dirtyTracked = true;
    profileSection.addEventListener("input", markProfileFormTouched);
    profileSection.addEventListener("change", markProfileFormTouched);
  }
  ...
  const cityEl = $("#prof-city");
  const emailDisplay = $("#prof-email-display");
  
  // Only populate input values if the user hasn't edited the form yet, or if force is true
  if (!profileFormTouched || force) {
    if (nameEl) nameEl.value = lawyerProfile.full_name || "";
    const currentBarNumber = (lawyerProfile.bar_number && !lawyerProfile.bar_number.startsWith("PENDING")) ? lawyerProfile.bar_number : "";
    if (barEl) barEl.value = currentBarNumber;

    // Email display
    const currentUser = LexAPI.getCurrentUser();
    if (emailDisplay) {
      emailDisplay.value = lawyerProfile.email || (currentUser ? currentUser.email : "");
    }
    ...
    // Practice courts
    const courts = (lawyerProfile.availability && lawyerProfile.availability.courts) || [];
    document.querySelectorAll("input[name='prof-courts']").forEach(chk => {
      chk.checked = courts.includes(chk.value);
      const chip = chk.closest(".court-chip");
      if (chip) chip.classList.toggle("selected", chk.checked);
    });
    ...
    // Address and City
    const rawAddr = lawyerProfile.practice_address || "";
    if (addressEl) {
      if (rawAddr.includes(" | City: ")) {
        const parts = rawAddr.split(" | City: ");
        addressEl.value = parts[0];
        if (cityEl) cityEl.value = parts[1];
      } else if (rawAddr.includes(", ")) {
        const parts = rawAddr.split(", ");
        if (cityEl && !cityEl.value) cityEl.value = parts[parts.length - 1];
        addressEl.value = rawAddr;
      } else {
        addressEl.value = rawAddr;
        if (cityEl && !cityEl.value) cityEl.value = "New Delhi";
      }
    }

    // Parse and sync structured State Bar Council inputs
    parseAndSetBarCouncilNumber(currentBarNumber);
  }

  // Update top profile header card dynamically
  const profAvatar = $(".profile-top .avatar");
  const profNameHeader = $(".profile-top h2");
  const avatarThumb = $("#avatar-thumbnail");
  if (profNameHeader) profNameHeader.textContent = lawyerProfile.full_name || "Advocate";
  if (profAvatar) {
    const name = lawyerProfile.full_name || "Advocate";
    const initials = name.split(" ").map(x => x[0]).join("").slice(0, 2).toUpperCase();
    profAvatar.textContent = initials;
    if (avatarThumb && !avatarThumb.querySelector("img")) {
      avatarThumb.textContent = initials;
    }
  }

  // Bank summary in Step 3
  const bankSummary = document.getElementById("bank-status-summary");
  if (bankSummary) {
    if (bankAccount && (bankAccount.account_number_masked || bankAccount.account_number)) {
      bankSummary.innerHTML = `
        <div style="display:flex;align-items:center;gap:8px;color:#15803d;font-weight:600;">
          <span>✓</span>
          <span>Linked: ${escapeHtml(bankAccount.bank_name || 'Bank Account')} (${escapeHtml(bankAccount.account_number_masked || '••••')}) · IFSC ${escapeHtml(bankAccount.ifsc_code || '')}</span>
        </div>
      `;
    } else {
      bankSummary.innerHTML = `
        <div style="color:var(--terra);font-size:13px;">
          <span>⚠ No payout account linked yet. Link your bank account for automated consultation earnings settlements.</span>
        </div>
      `;
    }
  }

  // Update dynamic completeness, checklists, and document upload buttons
  updateProfileCompletenessAndBadges();
}
...
async function handleSaveProfile(e) {
  ...
  const fullPracticeAddress = city && !address.toLowerCase().includes(city.toLowerCase()) ? `${address} | City: ${city}` : address;
  ...
  const selectedCourts = Array.from(document.querySelectorAll("input[name='prof-courts']:checked")).map(c => c.value);
  ...
  if (!name) {
    toast("Professional Name is a mandatory field.");
  ...
  if (!bar || bar.startsWith("PENDING")) {
    toast("State Bar Council Registration is mandatory. Please complete your registration details.");
    goToWizardStep(2, true);
    return;
  }
  if (!enrollmentDate) {
    toast("Date of Enrollment is mandatory.");
    goToWizardStep(2, true);
  ...
  if (!aadhaar) {
    toast("Aadhaar Number is a mandatory field for KYC.");
    goToWizardStep(3, true);
  ...
  if (!mobile) {
    toast("Mobile number is a mandatory field for KYC.");
    goToWizardStep(3, true);
  ...
  // Validate step 4 legal declarations
  const decl1 = document.getElementById("decl-good-standing");
  const decl2 = document.getElementById("decl-intermediary");
  const decl3 = document.getElementById("decl-confidentiality");

  if (decl1 && decl2 && decl3 && (!decl1.checked || !decl2.checked || !decl3.checked)) {
    toast("You must accept all three legal declarations to submit your profile.");
    goToWizardStep(4, true);
    return;
  }
  ...
  try {
    ...
    lawyerProfile.availability.courts = selectedCourts;
    lawyerProfile.availability.declarations_accepted_at = payload.availability.declarations_accepted_at;
    ...
    toast("Advocate onboarding details submitted successfully.");
    profileFormTouched = false;
    loadData();

    // Trigger SLA modal if credentials uploaded and pending review
    const slaModal = document.getElementById("review-sla-modal");
    if (slaModal && !lawyerProfile.verified) {
      setTimeout(() => {
...
async function uploadLawyerCredentialFile(file, docType, statusEl) {
  ...
  try {
    ...
    // Surgically update profile completeness bar and right sidebar badges without clearing form inputs
    if (typeof updateProfileCompletenessAndBadges === "function") {
      updateProfileCompletenessAndBadges();
    }
    if (typeof updateOverviewVerification === "function") {
      updateOverviewVerification();
    }
    ...
    try {
      ...
      if (docType === "profile_picture") lawyerProfile.profile_picture_url = mockKey;
  ...
  if (fileInput && listDiv) {
    fileInput.onchange = async () => {
      ...
      for (const file of files) {
        if (VR) {
          const check = VR.validateFile(file);
          if (!check.valid) {
            toast(check.error);
            continue;
          }
