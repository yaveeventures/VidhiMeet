...
## Corpus Check
- 102 files · ~166,897 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1798 nodes · 4802 edges · 81 communities (65 shown, 16 thin omitted)
...
### Community 2 - "je"
Cohesion: 0.06
Nodes (81): $(), aadhaarFileEl, ALL_TIME_SLOTS, barLicenceFileEl, bindTimeSelectListeners(), bookings, calculateExperience(), calculateProfileCompleteness() (+73 more)
...
### Community 6 - "main.py"
Cohesion: 0.07
Nodes (41): ntp_now(), ntp_now_ist(), NtpStatus, datetime, _query_ntp_server(), ntp_time.py ----------- NTP time synchronization for CERT-In and DPDP forensic t, Return the current time as a UTC-aware datetime adjusted by cached NPL/NIC NTP d, Return the current NPL/NIC-sourced time in Indian Standard Time (UTC+5:30). (+33 more)

### Community 7 - "i"
Cohesion: 0.09
Nodes (35): create_access_token(), hash_password(), Revoke a JWT by adding its jti to the revocation blocklist., Hash a raw password using Argon2id (OWASP #1 recommendation)., revoke_jti(), test_dispute_intermediary_shield(), test_dispute_workflow_matrix(), GET /api/v1/admin/ntp-status must return 200 with the expected keys for an admin (+27 more)

### Community 8 - "processEvent"
Cohesion: 0.06
Nodes (74): AsyncSession, DraftComment, DraftingProposal, DraftingRequest, LawyerProfile, User, admin_metrics(), get_admin_payouts() (+66 more)
...
Cohesion: 0.08
Nodes (27): Sanitize sensitive PII keys and credentials before log rendering., Configure structured JSON logging for production or key-value console logging fo, scrub_sensitive_pii_processor(), setup_logging(), FrontendStaticFiles, lifespan(), check_clock_drift(), _get_servers() (+19 more)

### Community 14 - "LawyerGrid"
Cohesion: 0.07
Nodes (51): get_settings(), rate_limit_dependency(), download_drafting_document(), drafting_document_mock_upload(), drafting_document_presign(), UploadFile, Helper to write a mock PDF file when serving local document downloads., _write_mock_pdf() (+43 more)
...
Cohesion: 0.18
Nodes (23): Voucher, booking_for_participant(), cancel_booking(), cancellation_preview(), complete_booking(), confirm_document(), confirm_payment(), create_booking() (+15 more)
...
Cohesion: 0.06
Nodes (24): Booking, now(), datetime, booking_ics(), _build_ics_calendar(), _build_vevent(), _escape(), _fmt_dt() (+16 more)
...
Cohesion: 0.07
Nodes (40): Base, get_db(), AuditLog, EncryptedString, Message, PasswordResetToken, RefreshToken, Review (+32 more)
...
- **Why does `User` connect `processEvent` to `test_password_reset.py`, `main.py`, `i`, `sanitize_key`, `LawyerGrid`, `8dcb01bed07f_initial_schema.py`, `PlatformFeedback`, `_e`, `80394484e25e_add_phonepe_transaction_id.py`?**
